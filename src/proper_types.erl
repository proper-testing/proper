...
%%%   instance of the specialized type. This will result in slower testing
%%%   -testing may even be stopped short, in case the `constraint_tries' limit
%%%   is reached (see the 'Options' section in {@link proper} module
%%%   documentation). If this is the case, it would be more appropriate to
%%%   generate valid instances of the specialized type using the ?LET macro.
%%%   Also make sure that even small instances can satisfy the constraint,
%%%   since PropEr will only try small instances at the start of testing. If
%%%   this is not possible, you can instruct PropEr to start at a larger size
%%%   (`start_size' option - see the 'Options' section in {@link proper} module
%%%   documentation for details).</p>
%%% </li>
%%% <li>`?SUCHTHATMAYBE(<X>, <Type>, <Condition>)'
%%%   <p>Equivalent to the `?SUCHTHAT' macro, but the constraint `<Condition>'
...
%%%   first tried in place of the failing instance.</p>
%%% </li>
%%% <li>`?LAZY(<Generator>)'
...
%%% Some types, e.g. unions, have no notion of size, thus their generation is
%%% not influenced by the value of `size'. The `size' parameter starts at `0'
%%% and grows automatically during testing.
%%% <ul>
...
%%%   type is generated recursively.</p>
%%% </li>
...
%%%   contained in `<Type>', such as the elements of a list - those will still
%%%   be generated using the test-wide value of `size'. One use of this
%%%   function is to modify types to grow faster or slower, like so:
